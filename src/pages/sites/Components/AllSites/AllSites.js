...
const AllSites = ({ deleteSiteHandler, allSiteState }) => {
  ...
  const deleteConfirm = (id) => {
    axios.delete('https://www.motkaml.online/wp-json/wp/v2/sites/deleteSite?site_id='+id).then((value) => {
      if (value.data === 1) {
        handleDeleteClose();
        deleteSiteHandler(id);
        deleteSwal.fire("تم حذف الموقع");
      }else{
        handleDeleteClose();
        deleteSwal.fire(value.status + "حدث خطأ ما حاول لاحقا");
      }
    }).catch((err)=>{
      handleDeleteClose();
      console.log(err);
      deleteSwal.fire("حدث خطأ ما حاول لاحقا");
